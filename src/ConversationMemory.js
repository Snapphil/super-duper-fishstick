...
function conductResearch(userQuestion, conversationContext) {
  ...
  console.log(`🔬 Plan: ${searches.length} searches, framework: ${plan.framework || 'general'}`);
    ...
    try {
      ...
      for (const fs of followUps.slice(0, 2)) {
        ...
        try {
          ...
          budget.gmail++;
        } catch (e) { console.warn(`Gap-fill search failed: ${e.message}`); }
      }
    } catch (e) {
      console.warn('Gap analysis failed: ' + e.message);
    }
  }
...
• searches MUST be a non-empty array`;
  ...
  return callGeminiJson_(`RESEARCH: ${question}`, sys, {
...
function analyzeGaps_(question, emails, searchLog, plan, conversationCtx) {
  ...
  return callGeminiJson_('Analyze gaps.', `Gap analysis. Q: ${question}. Hypothesis: ${plan.hypothesis || 'none'}.
  ...
  for (let i = 0; i < emails.length; i += CHUNK) {
    ...
    try {
      const result = callGemini_(
        ...
        'Extract relevant data. Precise. Bullet points.', { model: getConfig().GEMINI_MODEL, temperature: 0.1, maxTokens: 1024 }
      );
...
function quickResearch(searchQueries, userQuestion, conversationCtx, memory) {
  ...
  for (const sq of queries.slice(0, 3)) {
    ...
    try {
      ...
      emails.forEach(e => { if (!seen.has(e.threadId)) { allEmails.push(e); seen.add(e.threadId); } });
    } catch (e) { console.warn(`Quick search failed: ${e.message}`); }
  }
